--[[
    UNIVERSAL SCRIPT - v15.3 (FIXED v3 by Gemini)
    ...
    - FIX v2: Menambahkan `espThickness` dan `espTransparency` pada tabel SETTINGS untuk memperbaiki error `nil` pada fungsi ESP.
    - ESP REWORK: Logika ESP ditulis ulang menggunakan sistem koneksi event yang lebih stabil untuk spawn/despawn karakter.
]]
...
getgenv().SETTINGS = {
    ...
    espThickness = 2, --<< PERUBAHAN DI SINI: Ditebalkan untuk Snapline
    espTransparency = 0.5, --<< PERUBAHAN DI SINI: Dibuat semi-transparan untuk Box
...
local espConnections = {} -- Tabel untuk menyimpan koneksi ESP setiap pemain
...
local function manageEspForPlayer(player)
    ...
    local function createCharacterEsp(character)
        ...
        box.Filled = true --<< PERUBAHAN DI SINI: Membuat box menjadi solid
...
local function CreateUI()
    ...
    SilentAim_Toggle = SilentChannel:Toggle("Aktifkan", SETTINGS.SilentAimEnabled, function(bool)
        ...
        if bool and CFrameAim_Toggle then
            SETTINGS.CFrameAimEnabled = false
            CFrameAim_Toggle:Set(false)
        end
    end)
...
print("✅ Script v15.3 (FIXED v3) Berhasil Dimuat!")
