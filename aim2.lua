--[[
    UNIVERSAL SCRIPT - v15.3 (GOLD MASTER v12 by Gemini)
    - GOLD MASTER FIX: Memperbaiki race condition pada inisialisasi UI. Semua elemen UI kini dibuat dengan nilai default yang aman terlebih dahulu, baru kemudian diatur nilainya dari tabel SETTINGS. Ini adalah perbaikan paling stabil dan seharusnya menjadi solusi final.
]]
...
local function manageEspForPlayer(player)
    -- ... (Fungsi ESP tetap sama, tidak perlu diubah)
    print("DEBUG: Memulai manajemen ESP untuk " .. player.Name)
    local allDrawings = {}
    local connection = nil
    local function destroyEsp() if connection and connection.Connected then connection:Disconnect() end; connection = nil; for i, obj in ipairs(allDrawings) do obj:Remove() end; allDrawings = {} end
    local function createCharacterEsp(character)
        ...
        healthBar.Filled = true; box.Filled = true
        connection = runService.RenderStepped:Connect(function()
            if not character or not character.Parent or not isValidTarget(player) or not SETTINGS.ESPEnabled then for _, obj in ipairs(allDrawings) do obj.Visible = false end; return end
            ...
            if not onScreen then for _, obj in ipairs(allDrawings) do obj.Visible = false end; return end
            local boxSizeY, boxSizeX = 3000 / vector.Z, 2000 / vector.Z
            box.Visible = SETTINGS.ESPBox; if box.Visible then box.Size, box.Position = Vector2.new(boxSizeX, boxSizeY), Vector2.new(vector.X - boxSizeX / 2, vector.Y - boxSizeY / 2) end
            tracer.Visible = SETTINGS.ESPSnaplines; if tracer.Visible then tracer.From, tracer.To = Vector2.new(camera.ViewportSize.X / 2, camera.ViewportSize.Y), Vector2.new(vector.X, vector.Y) end
            name.Visible = SETTINGS.ESPNames; if name.Visible then name.Position, name.Text = Vector2.new(vector.X, vector.Y - boxSizeY/2 - 5), player.Name end
            healthBar.Visible = SETTINGS.ESPHealth; if healthBar.Visible then local hp = humanoid.Health / humanoid.MaxHealth; healthBar.Position = Vector2.new(vector.X - boxSizeX/2 - 7, vector.Y - boxSizeY/2); healthBar.Size = Vector2.new(5, boxSizeY * hp); healthBar.Color = Color3.fromHSV(hp / 3, 1, 1) end
        end)
    end
    player.CharacterAdded:Connect(createCharacterEsp); player.CharacterRemoving:Connect(destroyEsp); if player.Character then createCharacterEsp(player.Character) end
end
...
local function CreateUI()
    ...
    -- (CFrame Aim channel tidak diubah karena sudah berfungsi baik)
    local CFrameAim_Toggle = CFrameChannel:Toggle("Aktifkan", SETTINGS.CFrameAimEnabled, function(bool) SETTINGS.CFrameAimEnabled = bool end)
    local CFrameAim_FovSlider = CFrameChannel:Slider("FOV", 10, 500, SETTINGS.CFrameAimFov, function(val) SETTINGS.CFrameAimFov = val end)
    local CFrameAim_SmoothSlider = CFrameChannel:Slider("Smoothness", 0.01, 1, SETTINGS.CFrameAimSmoothness, function(val) SETTINGS.CFrameAimSmoothness = val end)
    local CFrameAim_PartDropdown = CFrameChannel:Dropdown("Target Part", {"Head", "Torso"}, function(selection) SETTINGS.CFrameAimHitbox = selection end)
    local CFrameAim_KeyDropdown = CFrameChannel:Dropdown("Tombol Hold", {"Right Click", "Left Click", "X"}, function(selection) SETTINGS.CFrameAimKey = selection end)
    
    CFrameAim_Toggle:Set(SETTINGS.CFrameAimEnabled)
    CFrameAim_PartDropdown:Set(SETTINGS.CFrameAimHitbox)
    CFrameAim_KeyDropdown:Set(SETTINGS.CFrameAimKey)


    local SilentChannel = AimServer:Channel("Silent Aim")
    --// << ====================== UI INITIALIZATION FIX DITERAPKAN DI SINI ======================
    
    -- 1. Buat elemen dengan nilai default yang aman
    local SilentAim_Toggle = SilentChannel:Toggle("Aktifkan", true, function(bool)
        SETTINGS.SilentAimEnabled = bool
        if not bool then cachedSilentAimTarget = nil end
    end)
    
    local keyOptions = {"Right Alt", "Left Alt", "Caps Lock", "Mouse Button 4", "Mouse Button 5"}
    local keyEnumMap = {
        ["Right Alt"] = Enum.KeyCode.RightAlt, ["Left Alt"] = Enum.KeyCode.LeftAlt, ["Caps Lock"] = Enum.KeyCode.CapsLock,
        ["Mouse Button 4"] = Enum.KeyCode.MouseButton4, ["Mouse Button 5"] = Enum.KeyCode.MouseButton5
    }
    local SilentAim_KeyDropdown = SilentChannel:Dropdown("Tombol Toggle", keyOptions, function(selection)
        if keyEnumMap[selection] then SETTINGS.SilentAimToggleKey = keyEnumMap[selection] end
    end)

    local SilentAim_FovSlider = SilentChannel:Slider("FOV", 10, 500, 130, function(val)
        SETTINGS.SilentAimFov = val
    end)

    local SilentAim_HitChanceSlider = SilentChannel:Slider("Hit Chance", 0, 100, 100, function(val)
        SETTINGS.SilentAimHitChance = val
    end)

    local SilentAim_PartDropdown = SilentChannel:Dropdown("Target Part", {"HumanoidRootPart", "Head", "Random"}, function(selection)
        SETTINGS.SilentAimHitbox = selection
    end)

    local SilentAim_MethodDropdown = SilentChannel:Dropdown("Metode", {"Raycast", "Mouse.Hit/Target", "FindPartOnRay"}, function(selection)
        SETTINGS.SilentAimMethod = selection
    end)

    -- 2. Setelah semua dibuat, baru atur nilainya dari SETTINGS
    SilentAim_Toggle:Set(SETTINGS.SilentAimEnabled)
    local defaultKeyName = "Right Alt"
    for keyName, keyCode in pairs(keyEnumMap) do if keyCode == SETTINGS.SilentAimToggleKey then defaultKeyName = keyName; break end end
    SilentAim_KeyDropdown:Set(defaultKeyName)
    SilentAim_FovSlider:Set(SETTINGS.SilentAimFov)
    SilentAim_HitChanceSlider:Set(SETTINGS.SilentAimHitChance)
    SilentAim_PartDropdown:Set(SETTINGS.SilentAimHitbox)
    SilentAim_MethodDropdown:Set(SETTINGS.SilentAimMethod)

    --// << ====================== AKHIR DARI UI FIX ======================

    -- (Sisa UI server tidak diubah karena tidak bermasalah)
...
local function setupEsp()
    ...
    local function connectPlayer(player) if player ~= localPlayer then pcall(manageEspForPlayer, player) end end
...
userInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == SETTINGS.SilentAimToggleKey then SETTINGS.SilentAimEnabled = not SETTINGS.SilentAimEnabled; if not SETTINGS.SilentAimEnabled then cachedSilentAimTarget = nil end end
end)
...
runService.RenderStepped:Connect(function()
    ...
    if updateCounter >= 5 then updateCounter = 0; if SETTINGS.SilentAimEnabled then cachedSilentAimTarget = getClosestPlayerForSilentAim() end end
    if fov_circle then fov_circle.Visible = SETTINGS.DrawFov; fov_circle.Color = SETTINGS.FovColor; fov_circle.Radius = SETTINGS.SilentAimEnabled and SETTINGS.SilentAimFov or SETTINGS.CFrameAimFov; fov_circle.Position = userInputService:GetMouseLocation() end
    if target_box then
        ...
        if target_box.Visible then
            ...
            if onScreen then target_box.Position = Vector2.new(screenPos.X - 4, screenPos.Y - 4); target_box.Color = SETTINGS.TargetColor else target_box.Visible = false end
        end
    end
end)
...
if hookmetamethod and getnamecallmethod then
    local oldNamecall; oldNamecall = hookmetamethod(game, "__namecall", function(...)
        ...
        if SETTINGS.SilentAimEnabled and self == workspace and not checkcaller() and CalculateChance(SETTINGS.SilentAimHitChance) then
            ...
            if hitPart then
                if SETTINGS.SilentAimMethod == "Raycast" and method == "Raycast" then if ValidateArguments(args, ExpectedArguments.Raycast) then args[3] = getDirection(args[2], hitPart.Position); return oldNamecall(unpack(args)) end
                elseif (SETTINGS.SilentAimMethod:find("FindPartOnRay")) and (method:find("FindPartOnRay")) then local ray = args[2]; if ray then args[2] = Ray.new(ray.Origin, getDirection(ray.Origin, hitPart.Position + (hitPart.Velocity * SETTINGS.SilentAimPredictionAmount))); return oldNamecall(unpack(args)) end end
            end
        end
        return oldNamecall(...)
    end)
    local oldIndex; oldIndex = hookmetamethod(game, "__index", function(self, index)
        if SETTINGS.SilentAimEnabled and self == mouse and not checkcaller() and SETTINGS.SilentAimMethod == "Mouse.Hit/Target" then
            ...
            if hitPart then
                ...
                if index:lower() == "hit" then if SETTINGS.SilentAimPrediction then return hitPart.CFrame + (hitPart.Velocity * SETTINGS.SilentAimPredictionAmount) else return hitPart.CFrame end end
            end
        end
...
print("✅ V12 (Gold Master) - UI Initialization FIXED! This should be the one.")
